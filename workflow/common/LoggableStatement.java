...
public class LoggableStatement implements PreparedStatement {
	...
	static String sqlLogFileName = "c:\\sip\\logs\\sql.log";
	static String sqlLogFileNameSlow = "c:\\sip\\logs\\sql_slow.log";
	static long sqlLogMaxSize = 128*1000*1000;
	static long MS_SQLSLOW = 15*1000;
	static java.text.SimpleDateFormat _sdf = new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss.SSS");
	...
	public boolean execute() throws java.sql.SQLException {
		if (sqlLogEnabled) {
			...
			boolean rc = wrappedStatement.execute();
			printLog(st, new java.util.Date());
			return rc;
		}
	...
	public java.sql.ResultSet executeQuery() throws java.sql.SQLException {
		if (sqlLogEnabled) {
			java.util.Date st = new java.util.Date();
			java.sql.ResultSet rc = wrappedStatement.executeQuery();
			printLog(st, new java.util.Date());
			return rc;
		}
	...
	public int executeUpdate() throws java.sql.SQLException {
		if (sqlLogEnabled) {
			java.util.Date st = new java.util.Date();
			int rc = wrappedStatement.executeUpdate();
			printLog(st, new java.util.Date());
			return rc;
		}
	...
	void printLog (java.util.Date stSta, java.util.Date stEnd) {
		long duration;
		String msg = String.format("%n%s\t%dms\t%s\t%s", _sdf.format(stSta), duration=stEnd.getTime()-stSta.getTime(), Thread.currentThread().getStackTrace()[3].getClassName(), getQueryString());
		try {
    		final Path path = Paths.get(sqlLogFileName);
    		if (Files.exists(path)) {
    			if (new java.io.File(sqlLogFileName).length()>sqlLogMaxSize) {
    				Files.move(path, path.resolveSibling("sql.log~"), StandardCopyOption.REPLACE_EXISTING);	
    			}
    		}
    		Files.write(path, Arrays.asList(msg), java.nio.charset.StandardCharsets.UTF_8,
        		Files.exists(path)? StandardOpenOption.APPEND : StandardOpenOption.CREATE);
		} catch (final java.io.IOException ioe) {
		}	
	  if (duration > MS_SQLSLOW)
		try {
    		final Path path = Paths.get(sqlLogFileNameSlow);
    		if (Files.exists(path)) {
    			if (new java.io.File(sqlLogFileNameSlow).length()>sqlLogMaxSize) {
    				Files.move(path, path.resolveSibling("sql_slow.log~"), StandardCopyOption.REPLACE_EXISTING);	
    			}
    		}
    		Files.write(path, Arrays.asList(msg), java.nio.charset.StandardCharsets.UTF_8,
        		Files.exists(path)? StandardOpenOption.APPEND : StandardOpenOption.CREATE);
    ...
    public void setNClob (int parameterIndex,
            java.io.Reader reader, long length)
              throws SQLException {
    ...
    public void setBinaryStream (int parameterIndex,
           java.io.InputStream inputStream)
             throws SQLException {
        wrappedStatement.setBinaryStream(parameterIndex, inputStream);
        saveQueryParamValue(parameterIndex, inputStream);
    }
    ...
    public void setBinaryStream (int parameterIndex,
           java.io.InputStream inputStream, long length)
             throws SQLException {
        wrappedStatement.setBinaryStream(parameterIndex, inputStream, length);
        saveQueryParamValue(parameterIndex, inputStream);
    }
    ...
    public void setAsciiStream (int parameterIndex,
           java.io.InputStream inputStream)
             throws SQLException {
        wrappedStatement.setAsciiStream(parameterIndex, inputStream);
        saveQueryParamValue(parameterIndex, inputStream);
    }
    ...
    public void setAsciiStream (int parameterIndex,
           java.io.InputStream inputStream, long length)
             throws SQLException {
        wrappedStatement.setAsciiStream(parameterIndex, inputStream, length);
        saveQueryParamValue(parameterIndex, inputStream);
    }
