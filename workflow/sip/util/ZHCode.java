...
public class ZHCode extends Encoding {
    ...
    public final static int QUESTIONMARK_HALFWIDTH = 3;
    public final static int QUESTIONMARK_FULLWIDTH = 4;
    public final static int TOTAL = 5;

    public final static String[] strategyNames = {
	"Delete", "HTML Escape", "Unicode Escape", "Question Mark(halfwidth)", "Question Mark(fullwidth)" };

    public final static String[] strategyDescriptions = {
	"Delete", "Use HTML Escape (e.g. &#x4e00;)", 
	"Use Unicode Escape (e.g. \\u4e00)", "Replace with Question Mark(halfwidth)", "Replace with Question Mark(fullwidth)" };
...
//	}
	...
	if (target_encoding == HZ) {
		...
		} else if (unsupportedStrategy == QUESTIONMARK_HALFWIDTH) {
		    // Unicode Escape \\uNNNN
		    dataline.replace(i, i+1, "?");
		}else if (unsupportedStrategy == QUESTIONMARK_FULLWIDTH) {
		    // Unicode Escape \\uNNNN
			 Transliterator transliterator = Transliterator.getInstance("Halfwidth-Fullwidth");
		    dataline.replace(i, i+1, transliterator.transliterate("?"));
		}
            }
        }
